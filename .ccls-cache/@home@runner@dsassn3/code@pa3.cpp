/*
    ...
    g++ -std=c++11 -o pa3.exe pa3.cpp sort.cpp utils.cpp tree.cpp avl.cpp bst.cpp closed_hash_function.cpp closed_hash_table.cpp open_hash_function.cpp open_hash_table.cpp
*/
#include <fstream>
#include <iostream>
#include <math.h>
...
/*
    ...
    Description:
        ...
        algorithm in ascending order. You can modify sort.cpp and sort.h files
   for this problem.
    ...
    Output:
        ...
        k-th position, including the initial state, separating the values with
   white space. Please use a built-in function to print the array. We won’t test
   array size over 20 or array size of 0.
...
/*
    ...
    Output:
        ...
        Print the array values at the initial state and at each iteration step
   where sorting is performed. Separate the values in the array with a space for
   each iteration step. You don’t need to consider exceptional cases such as
...
/*
    ...
    a.	Implement functions that inserts and deletes an element into a binary
   search tree (BST). Also you should find the sum of nodes that is larger than
   specific node value, and print the pre-order and in-order of the tree. You
   can modify bst.cpp and bst.h files for this problem.

    -	Input of nodes are integers from 1 to 99

    b.	Input & output of BinarySearchTree::insertion
        - Input: ('insertion',integer): Key of the element to be inserted. The
   key has a positive integer value.
        - Output: Return the -1 if the key already exists in the tree, 0
   otherwise. (If the key already exists, do not insert the element)
   ...
   If the key does not exist, do not delete any element Note that replace the
   smallest key in right subtree when delete the node with degree 2
    ...
    e.	task_3 prints
        i.	the return for each insertion/deletion for command (‘insertion’,
   integer)/ (‘deletion’, integer) and
        ii.	the return summation of nodes that is larger than a specific
   node value for command (‘sum’, integer) and
        iii.	the results of preorder and inorder traversal of the constructed
   tree for command (‘print’, NULL). (If empty tree, don’t return preorder or
   inorder traveral results)
*/
  ...
  try {
    ...
    for (int i = 0; i < instr_seq.getLength(); i++) {
      ...
      if (command.compare("insertion") == 0) {
        ...
        ret = tree.deletion(key, tree.get_root());
        fout << ret << endl;
      } else if (command.compare("sum") == 0) {
        ret = tree.sum(key, tree.get_root());
        fout << ret << endl;
...
/*
    ...
    Description:
        ...
        and report the frequency of each alphabet letter in a string using AVL
   tree operations. This involves handling imbalances that might occur during
   insertions and deletions, as well as ensuring that alphabet letters are
   counted accurately. As the string is received, insert each letter
   sequentially from the beginning to the end into the AVL tree.

        b.	AVL Tree Implementations
            - Modify or implement functions in the ‘AVLTree’ class that insert
   and delete elements (letters in this case).
   ...
   information in pre-order, in-order, and reverse-in-order manner.
            - Ensure the AVL tree can resolve imbalances (LL, LR, RL, RR) that
   might occur after modifications.
            - You can modify ‘avl.cpp’ and ‘avl.h’ files, and if needed, add
   public members to the ‘Node’ class implemented in ‘tree.h’.

        c.	Details of the process
            - Sequentially insert each letter from the string. If a node for
   that letter already exists, delete the existing node, increment the count,
   and re-insert the node.
            - During the deletion process, if a node has both children, use the
   replacement method where you find the minimum in the right subtree to replace
   the node
            - If counts are equal, prioritize based on the lexicographical order
   of the letters. For example, (c, 1) < (a, 4) < (b, 4).

        d.	Input & Output:
            - Input: A string consisting of at least one alphabetical letter.
   You don’t need to consider any other input cases and uppercase and lowercase
   of letters are same.
            - Output: (1) Frequency counts of letters, displayed from the most
   to the least frequent. If multiple letters have the same frequency, they
   should be displayed in lexicographical order.
            - Output: (2) The results of the preorder and inorder traversal of
   the constructed AVL tree after all operations.

        e.	task_4 prints
            i.	The results of reverse-inorder, inorder, preorder traversal of
   the constructed tree. ii.	The format of node for printing is “(letter,
   count)”
*/
...
/*
    ...
    Description:
        ...
        This hash table uses a singly linked list as a collision handling
   method. This hash table is used with integer keys and hashing into a table of
   size M. Every component of the key is folded with a size of 1(digit) and
   calculates their sum as described in our Lecture Note.
        ...
        After all operations are complete, print the total number of collisions
   that occurred and the length of the longest chain in the hash table.
...
/*
    ...
    Description:
        Implement insertion of a closed hash table with rehashing
   implementation. This hash table is used with integer keys and hashing into a
   table of size M. This hash table uses pseudo-random probing as a collision
   handling method. The index of the key k  after i-th collision, h_i (k), is:
        ...
        Please refer to slide “Example: Shift-Register Sequence” in “Hashing”
   Lecture Note for detail.

        Given M, k, d_1 for the shift-register sequence and inserted integers,
   print the result hash table. If the hash table is full or the collision
   cannot be resolved, stop the insertion, and print results of the insertions
   made up to that point, then print FAIL. Refer to instruction b & c for more
   detail.

        You don’t need to consider a deletion of a key, multiple insertions of
   the same key. You can modify closed_hash_function.cpp, closed_hash_table.cpp,
    ...
    Input: A sequence of commands
        ...
        - ('d',integer): the first probing offset for the shift-register
   sequence . (The third command is always 'd')
